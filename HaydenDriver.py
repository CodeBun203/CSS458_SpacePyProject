...
def main():
    ...
    DEFAULT_SIMULATION_DURATION_YEARS = 1 # Shorter for quick testing
    DEFAULT_TIME_STEP_MONTHS = 0.0001
    DEFAULT_DUMP_INTERVAL_SECONDS = getattr(SimIO, 'MIN_DUMP_TIME', 600.0) 

    try:
        initial_csv_files = [f for f in os.listdir(initial_systems_dir) if f.endswith("_Initial.csv")]
    ...
    initial_csv_files = ["Sun_Earth_Moon_Initial.csv"] # Override to test a single system
