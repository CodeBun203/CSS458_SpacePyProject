...
def dump_history(system_hist, sim_name, inital_time, final_time):
    ...
    * None 
    ...
    file_path = DEFAULT_DUMP_PATH + os.sep + sim_name + os.sep + str(inital_time)
...
#------------------------------ CSV Read Method -------------------------------
def reconstruct_history(sim_name):
    ...
    * A 2D numpy array of all the planets at each timestep that were dumped.
    ...
    while(not last_file):
        try:
            with open(file_path + str(cur_time_index), newline='') as csvfile:
...
def test_dump_and_recon():
    ...
    dump_history(history1, "Test_Sim", 0, 9)
    ...
    for i in range(0,10):
        ...
        for i in range(0, np.shape(system)[0]):
            system[i].update_pos(100)
    dump_history(history2, "Test_Sim", 10, 19)
    
    # reconstruct data
    full_hist = history1 + history2
    dumped_hist = reconstruct_history("Test_Sim")
